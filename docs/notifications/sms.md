...
**The null driver writes the number and the message to your log.** That is what it is for, and it is also customer contact details and an appointment time sitting in a file that erasing a booking does not reach — the erasure routine scrubs the booking and its related rows (see [GDPR & Erasure](Advanced-Gdpr-Data-Retention)), never `storage/logs`. Fine in development, and a disclosure you have to be able to make if you leave `sms` enabled without a gateway in production. Bind a driver that discards the body, or take `sms` back out of the channel list, if you cannot.
